...
static_assert(std::is_trivially_copyable<u32_dsp>(), "u32_dsp isn't trivially copyable");
...
// Excluding MSVC because it has weird behaviour for std::is_trivially_copyable.
    ...
    #define ASSERT_DSP_STRUCT(name, size) \
        static_assert(std::is_standard_layout<name>(), "DSP structure " #name " doesn't use standard layout"); \
        static_assert(std::is_trivially_copyable<name>(), "DSP structure " #name " isn't trivially copyable"); \
